...
const CLIENT_ID = "567679b13f32471fa75f72676a3ec037";
const CLIENT_SECRET = "363c5bcb877744169edda07cfd1f03c3";
...
function renderPlaylistsReel(allplaylists){
    ...
    allplaylists.forEach((playlistRowItem, index) => {
        console.log(playlistRowItem.playlists.items[0])

        const playListGridItemLinkElement = document.createElement("a");
        playListGridItemLinkElement.classList = "grid grid-cols-4 gap-[32px] gap-[10px] mt-[20px] flex";
        playListGridItemLinkElement.href = location.origin + `/pages/playlist.html?playlistId=${playlistRowItem.playlists.items[0]}`
        ...
        playlistRowItem.playlists.items.slice(0, 4).forEach(playlistItem => {
            ...
            `
            playListGridItemLinkElement.appendChild(playlistItemElement)
            playlistRowElementWrapper.appendChild(playListGridItemLinkElement);
        })
...
function navPlaylistsRender(navallPlaylists){
    ...
    navallPlaylists.forEach(playlist => {
        ...
        `;
        a.appendChild(navMenuPlayList)
        mainNavFragment.appendChild(a)
    })
