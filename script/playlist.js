...
const playerTrackName2 = document.querySelector("#player-track-name2");
...
function renderPlayListInfo(playlistData){
    ...
    tracks.forEach((trackInfo, index) => {
        ...
        trackItemElement.innerHTML = `
            ...
            <div class="flex gap-x-[10px] text-[20px]">
                <i data-liked-track="${trackInfo.track.id}" class="bi bi-heart-fill text-[#fff] text-[20px]"></i>
    ...
    ALL_LIKED_PLAYLISTS = JSON.parse(localStorage.getItem("likedTrack")) || [];

    playlistTracks.addEventListener("click", (e) => {
        if(e.target.closest(".bi-heart-fill")){
            const likedTrak = e.target.closest(".bi-heart-fill").dataset.likedTrack;
            fetch(`https://api.spotify.com/v1/audio-features/${likedTrak}`,{
                headers: {
                    "Authorization": ACCESS_TOKEN
                }
            }).then(response => response.json())
            .then(data => {
                const likeTrackColor = e.target.closest("[data-liked-track]");
                if(likeTrackColor){
                    let likeTrak = (likeTrackColor.toggleAttribute("true") ? likeTrackColor.style = 'color: #63CF6C': likeTrackColor.style = 'color:#fff');
                    ALL_LIKED_PLAYLISTS.unshift(data)
                    localStorage.setItem("likedTrack", JSON.stringify(ALL_LIKED_PLAYLISTS)); 
                }
            })
        }
    })
    ...
    function renderPlayerInfo(currentPlayingTrackNumber){
        ...
        if(playingTrack.track.name.length < 15){
            playerTrackName2.innerHTML = playingTrack.track.name
        }else{
        ...
        playerTrackImage.addEventListener("click", (e) => {
            console.log(e.target);
            if(e.target){
                playerTrackImage.toggleAttribute("true") ? playerTrackImage.style = "height:15px; transform:translateY(290px);" : playerTrackImage.style = "height:300px; transform:translateY(0);"
            }
        })
    }
    renderPlayerInfo(+localStorage.getItem("currentAudioNumber"))
